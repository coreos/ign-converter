...
// Check2_2 returns if the config is translatable but does not do any translation.
// fsMap is a map from v2 filesystem names to the paths under which they should
// be mounted in v3.
func Check2_2(cfg old.Config, fsMap map[string]string) error {
...
func translatePartitions(parts []old.Partition) (ret []types.Partition) {
	for _, p := range parts {
		ret = append(ret, types.Partition{
			Label:    util.StrP(p.Label),
			Number:   p.Number,
			SizeMiB:  util.IntP(p.Size),
			StartMiB: util.IntP(p.Start),
			TypeGUID: util.StrP(p.TypeGUID),
			GUID:     util.StrP(p.GUID),
			// use the default value from the spec
			// for the two following attributes.
			WipePartitionEntry: util.BoolP(false),
